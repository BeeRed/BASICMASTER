...
*
DISP    EQU     $A094           ; I/OのDISPLAYポインタ
*
************************
* SYSTEM ADRS
*
STACK   EQU     $A047           ; スタック (RAMEND)
IXSTCK  EQU     $A080
VRAM    EQU     $C0             ; VRAM ADRS
PIACRB  EQU     $E006           ; H68 KEYSCAN用
*
************************
* BASIC WORK
*
        ORG     $0
VTOP    RMB     (26+1)*2        ; ==54 変数エリア dummy,'A'〜'Z'
BOP     RMB     2               ; システム変数 '['
EOP     RMB     2               ; '\'
MEMEND  RMB     2               ; ']'
...
STOP    JSR     PRINT           ; STOP処理
        ...
        BRA     END1
*
CSTART  JSR     INI4            ; COLD START INITILIZE for H68
        LDX     BGNTXT
        STX     BOP
        LDX     ENDTXT
        STX     MEMEND
...
C3      CLR A                       ; 引数からADRS計算
        ...
        ROR B
        ROR B
        ADC A   #VRAM               ; V-RAM 開始ADRSの
        RTS                         ; 　上位1バイト
*
ER9     JMP     ERR9
*                                   ;"CURS(X , Y)"
CURS    BSR     SUB10               ;   AccA   AccB
        BSR     SUB2                ; 引数
        CMP A   #32                 ; WIDTH??? ; CURSORの範囲チェック
        BCC     ERR16
        CMP B   #16
        BCC     ERR16
        STA A   CHCT
...
K0      JSR     SUB6                ; BM版では引数に意味はないが残しておく
        ...
        JSR     CMP
        INC B
K1      STA     PIACRB              ; AccA←Code, C=1 if pressed
        PSH A
        LDA A   PIACRB
        CMP     #$55
        BLS     K2
...
*
ERROR   FCB     'E','R','R','O','R',' ','N','O','.',0
*
READY   FCB     $D,$A,'R','E','A','D','Y',7,0
*
CRLF    FCB     $D,$A,0
*
REENT   FCB     'R','E','-','E','N','T','E','R',0
*                                   ;［注:TABLEの各KEY WORDの
...
COMAND  EQU     *-2
        ...
        FDB     RUN
        FCB     'E','X'+Z
        FDB     MONITOR_ENTRY       ;⇦MONITORのADRS
        FCB     'A','U','T','O'+Z
        FDB     AUTO
        FCB     'N','E','W'+Z
        FDB     NEW
...
PROG    EQU     *
        END
;
; vim: set hlsearch expandtab smarttab ts=8 noai:
;
